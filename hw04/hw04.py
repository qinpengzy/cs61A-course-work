...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        left_arm = left(m)
        right_arm = right(m)
        left_subtree = totals_tree(end(left_arm))
        right_subtree = totals_tree(end(right_arm))
        total_weights = total_weight(end(left_arm)) + total_weight(end(right_arm))
        return tree(total_weights, [left_subtree, right_subtree])
    else:
        return tree(0)
...
def has_path(t, phrase):
    ...
    if label(t) == phrase:
        return True
    elif is_leaf(t) or len(phrase) == 1:
        return False
    else:
        for b in branches(t):
            if label(t) == phrase[0] and has_path(b, phrase[1:]):
                return True
        return False
